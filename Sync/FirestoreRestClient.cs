...
namespace TaskMate.Sync {
    ...
    public sealed class FirestoreRestClient {
        ...
        public async Task<bool> DocExistsAsync(string docPath) {
            var url = WithKey($"{BaseUrl}/documents/{docPath}");

            using var res = await SendAsync(() => Build(HttpMethod.Get, url, body: null))
            ...
            if(!res.IsSuccessStatusCode) {
                ...
                throw new HttpRequestException(
                    $"Firestore error {(int)res.StatusCode}: {res.ReasonPhrase}\n{payload}");
            }
